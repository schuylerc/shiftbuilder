...
class schedule_model extends CI_Model {
	...
	public function bestCanidate($shift, $users) {
		...
		$shift_type = $this->get_shift_type ( $shift ); // what type the shift is, ie morning/afternoon
		...
		foreach ($users as $user ) {
			...
			$userpref = $this->get_user_preference ( $user, $shift_type );
	...
	/*
	 * function scheduled_hours_sort($users){ //returns a sorted list of users based on their scheduled hours, the most hours goes first //echo $users; usort($users, 'sortByHoursRemaining'); //echo $users; return $users; } function sortByHoursRemaining($a, $b) { $a_hours_remain = ($a->max_hours - $a->hours_scheduled); $b_hours_remain = ($b->max_hours - $b->hours_scheduled); if ($a_hours_remain == $b_hours_remain){return 0;} if($a_hours_remain > $b_hours_remain){return 1;} if($a_hours_remain < $b_hours_remain){return -1;} }
	 */
